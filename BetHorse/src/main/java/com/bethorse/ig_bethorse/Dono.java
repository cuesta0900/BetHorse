...
public class Dono {
    ...
    public void trocarSenha(Connection con, long cpf, String oldSenha, String senha){
        try{
            if((oldSenha.equals(senha)) || senha.equals("".trim())){
                ...
                String sql = "UPDATE DONO SET senhaDono = '"+senha+"' where cpfDono = " + cpf + ";";
                PreparedStatement pstm = con.prepareStatement(sql);
                pstm.executeUpdate();
    ...
    public void trocarSenhaAp(Connection con, long cpf, String oldSenha, String senha){
        try{
            if((oldSenha.equals(senha)) || senha.equals("".trim())){
                JOptionPane.showMessageDialog(null, "Invalidade na nova senha", "Erro!", JOptionPane.ERROR_MESSAGE);
            }else{
                String sql = "UPDATE apostador SET senhaAp = '"+senha+"' where cpfApostador = " + cpf + ";";
                PreparedStatement pstm = con.prepareStatement(sql);
                pstm.executeUpdate();
                JOptionPane.showMessageDialog(null, "Senha alterada com sucesso!");
            }
    ...
    public String mostrarSenha(Connection con, long cpf, String senha){
        String newsenha = "";
        try{
            String query = "SELECT senhaDono from dono where cpfDono = ?";
            PreparedStatement ptsm = con.prepareStatement(query);
            ptsm.setLong(1, cpf);
            ResultSet rs = ptsm.executeQuery();
            if(rs.next()){
                newsenha = rs.getString("senhaDono");
            }
        }catch(Exception e){
            JOptionPane.showMessageDialog(null,"Erro!!\n" + e, "Erro!", JOptionPane.ERROR_MESSAGE);
        }
    ...
    public String mostrarSenhaAp(Connection con, long cpf, String senha){
        String newsenha = "";
        try{
            String query = "SELECT senhaAp from apostador where cpfApostador = ?";
            PreparedStatement ptsm = con.prepareStatement(query);
            ptsm.setLong(1, cpf);
            ResultSet rs = ptsm.executeQuery();
            if(rs.next()){
                newsenha = rs.getString("senhaAp");
            }
        }catch(Exception e){
            JOptionPane.showMessageDialog(null,"Erro!!\n" + e, "Erro!", JOptionPane.ERROR_MESSAGE);
        }
